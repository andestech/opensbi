...
typedef struct acpi_ghes_state {
...
typedef struct acpi_ghes_error_info {
	uint32_t etype;
	union {
		struct {
...
typedef struct __attribute__((__packed__)) acpi_gas {
	uint8_t asid;
	uint8_t reg_bwidth;
	uint8_t reg_boffs;
	uint8_t access_sz;
...
typedef struct __attribute__((__packed__)) acpi_ghes_notif {
	uint8_t type;
	uint8_t length;
	uint16_t config_we;
...
typedef struct __attribute__((__packed__)) acpi_ghes {
	uint16_t type;
	uint16_t src_id;
	uint16_t rel_id;
	uint8_t flags;
...
typedef struct __attribute__((__packed__)) acpi_ghesv2 {
	acpi_ghes ghes;
	acpi_gas ack_reg;
...
typedef struct __attribute__((__packed__)) acpi_ghes_section_type {
	uint8_t type[16];
 ...
 * +------------------------------------+
 */
typedef struct __attribute__((__packed__)) acpi_ghes_cper_header {
	uint8_t sign[4];
	uint16_t rev;
	uint32_t sign_end;
...
typedef struct __attribute__((__packed__)) acpi_ghes_cper_section_desc {
...
typedef struct __attribute__((__packed__)) acpi_ghes_gen_proc_sec {
...
typedef struct __attribute__((__packed__)) acpi_ghes_cper_mem2_sec {
	uint64_t vbits;
	uint64_t err_status;
	uint64_t phys_addr;
...
typedef union __attribute__((__packed__)) acpi_ghes_cper_section {
	cper_gen_proc_sec ps;
	cper_mem2_sec ms;
} cper_section;

/* CPER Record */
typedef struct __attribute__((__packed__)) acpi_ghes_cper {
	cper_header header;
	cper_section_desc sec_descs[MAX_SECS_PER_REC];
	cper_section sections[MAX_SECS_PER_REC];
} acpi_ghes_cper;

typedef struct __attribute__((__packed__)) acpi_ghes_data_entry {
...
typedef struct __attribute__((__packed__)) acpi_ghes_status_block {
